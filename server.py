    ...
    def handle_commands(self, mysql_connection, mysql_cursor):
        while True:
            ...
            if data.startswith("download_folder"):
                ...
                add_to_waiting_commands(get_users_user_is_sharing_with(self.username), data)
                ...
                if self.username in waiting_commands:
                    self.client_socket.send(fernet.encrypt(dumps(waiting_commands[self.username])))
                    waiting_commands[self.username] = []
                else:
                    self.client_socket.send(fernet.encrypt(dumps([])))

            else:
                self.client_socket.send(fernet.encrypt("Invalid command".encode()))

    def send_command_to_shared_users(self, command):
        shared_users = get_users_user_is_sharing_with(self.username)
        for shared_user in shared_users:
            shared_user_thread: ClientThread = get_client_thread_by_username(shared_user)
            if shared_user_thread is not None:
                shared_user_thread.client_socket.send(fernet.encrypt(command.encode()))
...
def get_client_thread_by_username(username):
    for client_thread in client_threads:
        if client_thread.username == username:
